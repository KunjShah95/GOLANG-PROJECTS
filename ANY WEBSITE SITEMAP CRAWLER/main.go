...
// WriteSeoDataToFile writes the SEO data to a .txt file
func WriteSeoDataToFile(data []SeoData, filename string) error {
	var content strings.Builder
	for _, d := range data {
		content.WriteString(fmt.Sprintf("URL: %s\nTitle: %s\nH1: %s\nMetaDescription: %s\nStatusCode: %d\n\n", d.URL, d.Title, d.H1, d.MetaDescription, d.StatusCode))
	}
...
func main() {
	...
	err := WriteSeoDataToFile(results, "seo_data.txt")
	if err != nil {
		fmt.Println("Error writing SEO data to file:", err)
	} else {
		fmt.Println("SEO data written to file successfully.")
	}
}
